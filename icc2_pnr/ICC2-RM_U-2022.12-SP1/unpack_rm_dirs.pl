...
# Version: R-2020.09 
# Copyright (C) 2016-2020 Synopsys, Inc. All rights reserved.
##########################################################################################
...
if (! -d "rm_setup" || ! -d "rm_icc2_dp_hier_scripts" || ! -d "rm_icc2_dp_flat_scripts" || ! -d "rm_icc2_pnr_scripts") {
...
for my $name (keys %blocks) {
	...
	system("cp -r ./rm_icc2_dp_hier_scripts $install_dir/dp/$name");
	system("cp -r ./rm_icc2_dp_flat_scripts $install_dir/dp/$name");
	...
	if ($blocks{$name}{style} =~ /flat/) {
	    ...
	    system("rm -rf rm_icc2_dp_hier_scripts");
	} else {
	    system("rm rm_setup/Makefile_dp_flat");	    
	    system("rm -rf rm_icc2_dp_flat_scripts");	    
	}
...
##	system("rm rm_setup/icc2_pnr_setup.tcl");
    ...
    while (<$infile>) {
	## Substitute for either "" or [list ] style of listing block references
        s/(set\s+USE_ABSTRACTS_FOR_BLOCKS\s+)"[ a-zA-Z0-9_]*?"(.*)/\1"$blocks{$name}{descendants}"\2/;
        s/(set\s+USE_ABSTRACTS_FOR_BLOCKS\s+)\[list[ a-zA-Z0-9_]*?\](.*)/\1\[list $blocks{$name}{descendants}\]\2/;
