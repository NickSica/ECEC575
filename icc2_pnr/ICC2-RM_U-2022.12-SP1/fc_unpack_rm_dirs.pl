...
if ($ARGV[0] =~ /(-h|-help|--help)/ || $#ARGV != 1) {
    ...
    print "\nPlease read README.FC-UNPACK-RM.txt in the FC RM scripts directory for more usage instructions.\n";
    die "\n";
}
...
if (! -d "rm_setup" || ! -d "rm_fc_dp_hier_scripts" || ! -d "rm_fc_dp_flat_scripts" || ! -d "rm_fc_scripts") {
...
while (<FILE>) {
    ...
    if (!/^(\t*| *)[A-Za-z0-9_]+$/) {
	chomp $_;
	die "\nIllegal format for line: $_\nRefer to README.FC-UNPACK-RM.txt for explanation of design hierarchy configuration file format.\n";
    }
...
for my $name (keys %blocks) {
	...
	system("cp -r ./rm_fc_dp_hier_scripts $install_dir/dp/$name");
	system("cp -r ./rm_fc_dp_flat_scripts $install_dir/dp/$name");
	system("cp -r ./rm_fc_scripts $install_dir/dp/$name");
	system("cp -r ./rm_user_plugin_scripts $install_dir/dp/$name");
	system("ln -s ../../$rm_script_dir[-1]/rm_tech_scripts $install_dir/dp/$name/rm_tech_scripts");
	system("ln -s ../../$rm_script_dir[-1]/rm_utilities $install_dir/dp/$name/rm_utilities");

	## Populate $DESIGN_NAME, $DESIGN_STYLE, $PHYSICAL_HIERARCHY_LEVEL, $RELEASE_DIR_DP, $RELEASE_DIR_PNR in $install_dir/dp/$name/rm_setup/design_setup.tcl
	system("mv $install_dir/dp/$name/rm_setup/design_setup.tcl $install_dir/dp/$name/rm_setup/design_setup.temp");
	open my $infile, '<',  "$install_dir/dp/$name/rm_setup/design_setup.temp" or die "Can't open temporary RM script file $install_dir/dp/$name/rm_setup/design_setup.temp to parse\n";
	open my $outfile, '>', "$install_dir/dp/$name/rm_setup/design_setup.tcl" or die "Can't open RM script file $install_dir/dp/$name/rm_setup/design_setup.tcl to write changes\n";
	while (<$infile>) {
	...
	close $infile;
	close $outfile;
	system("rm $install_dir/dp/$name/rm_setup/design_setup.temp");
	
	## Populate $DP_BLOCK_REFS and $DP_INTERMEDIATE_LEVEL_BLOCK_REFS in $install_dir/dp/$name/rm_setup/fc_dp_setup.tcl
	system("mv $install_dir/dp/$name/rm_setup/fc_dp_setup.tcl $install_dir/dp/$name/rm_setup/fc_dp_setup.temp");
	open my $infile, '<',  "$install_dir/dp/$name/rm_setup/fc_dp_setup.temp" or die "Can't open temporary RM script file $install_dir/dp/$name/rm_setup/fc_dp_setup.temp to parse\n";
	open my $outfile, '>', "$install_dir/dp/$name/rm_setup/fc_dp_setup.tcl" or die "Can't open RM script file $install_dir/dp/$name/rm_setup/fc_dp_setup.tcl to write changes\n";
	while (<$infile>) {
	...
	close $infile;
	close $outfile;
	system("rm $install_dir/dp/$name/rm_setup/fc_dp_setup.temp");
	...
	if ($blocks{$name}{style} =~ /flat/) {
	    ...
	    system("rm -rf rm_fc_dp_hier_scripts");
	} else {
	    system("rm rm_setup/Makefile_dp_flat");
	    system("rm -rf rm_fc_dp_flat_scripts");
	}
	system("rm rm_setup/Makefile");
##	system("rm rm_setup/fc_setup.tcl");
	chdir $orig_dir;
    ...
    system("cp -r ./rm_fc_scripts $install_dir/pnr/$name");
    system("cp -r ./rm_user_plugin_scripts $install_dir/pnr/$name");    
    system("ln -s ../../$rm_script_dir[-1]/rm_tech_scripts $install_dir/pnr/$name/rm_tech_scripts");
    system("ln -s ../../$rm_script_dir[-1]/rm_utilities $install_dir/pnr/$name/rm_utilities");

    ## Populate $DESIGN_NAME, $DESIGN_STYLE, $PHYSICAL_HIERARCHY_LEVEL, $RELEASE_DIR_DP, $RELEASE_DIR_PNR in $install_dir/pnr/$name/rm_setup/design_setup.tcl
    system("mv $install_dir/pnr/$name/rm_setup/design_setup.tcl $install_dir/pnr/$name/rm_setup/design_setup.temp");
    open my $infile, '<',  "$install_dir/pnr/$name/rm_setup/design_setup.temp" or die "Can't open temporary RM script file $install_dir/pnr/$name/rm_setup/design_setup.temp to parse\n";
    open my $outfile, '>', "$install_dir/pnr/$name/rm_setup/design_setup.tcl" or die "Can't open RM script file $install_dir/pnr/$name/rm_setup/design_setup.tcl to write changes\n";
    while (<$infile>) {
	s/(set\s+DESIGN_NAME\s+)"[ a-zA-Z0-9_]*?"(.*)/\1"$name"\2/;
        s/(set\s+DESIGN_STYLE\s+)"[ a-z]*?"(.*)/\1"$blocks{$name}{style}"\2/;
        s/(set\s+PHYSICAL_HIERARCHY_LEVEL\s+)"[ a-z]*?"(.*)/\1"$blocks{$name}{level}"\2/;
    ...
    close $infile;
    close $outfile;
    system("rm $install_dir/pnr/$name/rm_setup/design_setup.temp");

    ## Populate $USE_ABSTRACT_FOR_BLOCKS in $install_dir/pnr/$name/rm_setup/fc_setup.tcl
    system("mv $install_dir/pnr/$name/rm_setup/fc_setup.tcl $install_dir/pnr/$name/rm_setup/fc_setup.temp");
    open my $infile, '<',  "$install_dir/pnr/$name/rm_setup/fc_setup.temp" or die "Can't open temporary RM script file $install_dir/pnr/$name/rm_setup/fc_setup.temp to parse\n";
    open my $outfile, '>', "$install_dir/pnr/$name/rm_setup/fc_setup.tcl" or die "Can't open RM script file $install_dir/pnr/$name/rm_setup/fc_setup.tcl to write changes\n";
    while (<$infile>) {
	## Substitute for either "" or [list ] style of listing block references
	...
	s/(set\s+SUB_BLOCK_REFS\s+)"[a-zA-Z0-9_]*?"(.*)/\1"$blocks{$name}{children}"\2/;
	s/(set\s+SUB_BLOCK_REFS\s+)\[list[ a-zA-Z0-9_]*?\](.*)/\1\[list $blocks{$name}{children}\]\2/;
        s/(set\s+INIT_DESIGN_INPUT\s+)"[a-zA-Z0-9_]*?"(.*)/\1"DP_RM_NDM"\2/;
	print $outfile $_;
    }
    close $infile;
    close $outfile;
    system("rm $install_dir/pnr/$name/rm_setup/fc_setup.temp");

    ## Link DP Makefile, and delete PNR related files
    chdir "$install_dir/pnr/$name";
    system("ln -s rm_setup/Makefile ./Makefile");
    system("rm rm_setup/Makefile_dp_flat");
    system("rm rm_setup/Makefile_dp_hier");
    system("rm rm_setup/fc_dp_setup.tcl");
    chdir $orig_dir;
